(function ($) {
	...
	const onKeyDownSwatches = (event) => {
		...
		if (['Left', 'ArrowLeft'].includes(key)) {
			...
			setSelectedIndex($el, selectedIndex - 1);
			$el.trigger('input');
			...
			event.preventDefault();
			setSelectedIndex($el, selectedIndex - Math.floor(count / rows));
			$el.trigger('input');
	...
	const onClickSwatches = (event) => {
		const target = event.target;
		const $el = $(target.closest('.ui_swatches'));
		
		if (target.classList.contains('swatch')) {
			const {swatches} = $el.data();
			setSelectedIndex($el, swatches.indexOf(target));
			$el.trigger('input');
		}
	};

	const setSelectedIndex = ($el, index) => {
	...
	$.fn.uiSwatches = function(behavior, ...args) {
		...
		for (let i = 0; i < this.length; i++) {
			...
			if (Object.prototype.toString.call(behavior) !== '[object String]') {
				...
				$el
					.on('click', onClickSwatches)
					.on('keydown', onKeyDownSwatches);
			}
			// Behaviors
			else if (behavior === 'setSelectedHex') {
				const newValue = args[0] + '';
				setSelectedHex($(el), newValue);
			} else if (behavior === 'getSelectedHex') {
				const { selectedIndex, swatches } = $(el).data();
				returnValues.push($(swatches[selectedIndex]).data('hex'));
			} else if (behavior === 'setAllHex') {
				setAllHex($(el), args[0]);
			} else if (behavior === 'getAllHex') {
				const {swatches} = $(el).data();
